...
class CAM_Module(nn.Module):
    def __init__(self,c1,c2,k=1,s=1):
        ...
        self.conv1 = nn.Conv2d(c1, c2, kernel_size=k, stride=s)  #
        self.gamma = nn.Parameter(torch.tensor(0.5, dtype=torch.float),requires_grad=True)  #
    ...
    def forward(self, x):
        ...
        # X -> (N,C',C)
        energy = torch.bmm(proj_query, proj_key)  # [B,256,HW][B,HW,2048]=[B,256,2048]
        #
        # proj_key = x.view(m_batchsize, C, -1).permute(0, 2, 1).contiguous()
        # proj_key = proj_key.reshape(m_batchsize, height * width, 2, C // 2)
        # energy = torch.einsum('ijk,iklm->ijlm', (proj_query, proj_key))
        # energy = energy.permute(0, 2, 1, 3).contiguous()
        #
        #
        # expand_as
        energy /= (proj_query.shape[-1]**0.5)
        attention = self.softmax(energy)   #
        # attention = attention.permute(0, 2, 1, 3).contiguous().view(m_batchsize, C1, -1)
        #dropuot
...
class ACConv(nn.Module):
    ...
    def __init__(self, in_channels,c2,kernel_size=3, stride=1, group=1,dilation=1):
    ...
    def forward(self, x):
        sigma = self.conv(x)  # [b,k*k,h,w] 用一个卷集层，维持HW不变的PAD后进行卷集，将通道变成C-》2*K*K
        #sigma = self.conv(self.pad(x))
        sigma = self.bn(sigma)  # 32*9=288
        sigma = self.softmax(sigma)   #是否使用softmax
        # sigma = self.sigmoid(sigma)     #通道上进行softmax， K*K值和为1，保证地通且不改变亮度
        # sigma = self.relu(sigma)

        n, c, h, w = sigma.shape  # C=2*K*K  18

        sigma = sigma.reshape(n, self.group, c//self.group, h * w)  # [n,1,18,h*w]：1表示同一组的特征的不同channel，都使用相同的weight
        ...
        out = torch.sum(x*sigma, dim=3).reshape(n,c1,h,w)  #在卷集和的维度上对x*sigma求和相当于卷集操作
...
# add by myself
# from utils.GCNet import ContextBlock
# class C3GC(nn.Module):
#     def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5): # ch_in, ch_out, number, shortcut, groups, expansion
#         super(C3GC, self).__init__()
#         c_ = int(c2 * e) # hidden channels
#         self.gc = ContextBlock(c1)
#         self.cv1 = Conv(c1, c_, 1, 1)
#         self.cv2 = Conv(c1, c_, 1, 1)
...
if __name__ == '__main__':
    ...
    # out = cam(input)
    # adc = CAM_Module(1280,640)
    # input = torch.randn(1,1280,20,32)
    # out = adc(input)
    adc = ACConv(640, 640)
    input = torch.randn(1, 640, 20, 32)
    out = adc(input)