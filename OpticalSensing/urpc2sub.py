...
def coco2vistxt(json_path, out_folder):
    ...
    for i in tqdm(range(len(labels))):
        file_name = str(labels[i]['image_id']).zfill(6)
        # pdb.set_trace()
        ...
        with open(os.path.join(out_folder, file_name), 'a+', encoding='utf-8') as f:
            ...
            line = str(underwater_classes[labels[i]['category_id']]) + ',' + str(s)[1:-1].replace(' ', '') + ',' + ('%.2f'%float(labels[i]['score']))
            f.write(line+'\n')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='coco test json to visdrone txt file.')
    if not os.path.exists('/home/user7/Desktop/answerB/sub_optics/'):
        os.mkdir('/home/user7/Desktop/answerB/sub_optics/')
    shutil.rmtree('/home/user7/Desktop/answerB/sub_optics/') 
    os.mkdir('/home/user7/Desktop/answerB/sub_optics/') 
    parser.add_argument('-j', help='JSON file', default='/home/user7/Desktop/test_run_dir/optics/water_m6_last_predictions.json')
    parser.add_argument('-o', help='path to output folder', default='/home/user7/Desktop/answerB/sub_optics/')
