...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16):
    ...
    # Sort by objectness
    i = np.argsort(-conf)   # 返回排序后的索引,大到小
    ...
    for ci, c in enumerate(unique_classes): # 对每一个类别进行遍历处理
        i = pred_cls == c  # i: 记录着所有预测框是否是c类别框   是c类对应位置为True, 否则为False
        n_l = nt[ci]  # number of labels，gt框中的c类别框数量
        n_p = i.sum()  # number of predictions，预测框中c类别的框数量
        if n_p == 0 or n_l == 0:
            continue

        # Accumulate FPs and TPs
        fpc = (1 - tp[i]).cumsum(0)   # cusum：累加操作
        tpc = tp[i].cumsum(0)
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        for i, gc in enumerate(gt_classes): # 遍历gt
            j = m0 == i # 
            if n and sum(j) == 1:   # 如果预测的正确  self.matrix：[num_classes,num_classes]
